...
def _get_retry_delay(error: urllib.error.HTTPError, attempt: int) -> float:
    """
...
def _urlopen_with_retry(
    ...
    for attempt in range(_MAX_RETRIES + 1):
        try:
            ...
            logger.warning(
                f"Retrying OpenAI request in {delay:.2f}s "
                f"because of HTTP {e.code}"
            )
...
def _fetch_openai_costs(admin_key: str) -> list:
    ...
    start_dt = now_dt - timedelta(
        days=_DEFAULT_LOOK_BACK_DAYS
    )
    ...
    try:
        ...
        logger.info(
            f"Fetched {len(data)} OpenAI cost buckets"
        )
...
def _fetch_openai_completions(admin_key: str) -> list:
    ...
    while True:
        ...
        if page_token:
            current_url = (
                f"{base_url}&page={page_token}"
            )

        logger.info(
            f"Fetching OpenAI completions "
            f"page={page_count}"
        )
        ...
        try:
            ...
            logger.error(
                f"OpenAI completions API error "
                f"{e.code}: {error_body}"
            )
            ...
            logger.error(
                f"Failed to fetch OpenAI completions: "
                f"{str(e)}",
                exc_info=True,
            )
        ...
        logger.info(
            f"Completion page={page_count} "
            f"returned {len(page_data)} buckets"
        )
        ...
        if has_more and next_page:
            ...
            logger.info(
                f"More completion pages available. "
                f"next_page={page_token}"
            )
        else:
            break
...
def _build_snapshot(
    ...
    current_month_start_date = today.replace(
        day=1
    )
...
def _post_snapshot(
    ...
    target_url = (
        f"{url}/api/v1/usage-api/openai/snapshot"
    )

    logger.info(
        f"Posting snapshot to {target_url}..."
    )
    ...
    # We don't use OpenAI retry logic here.
...
def lambda_handler(event, context):
    ...
    logger.info(
        f"OpenAI Usage Snapshot Lambda started. "
        f"request_id={request_id}"
    )

    logger.info(
        f"Lookback days: {_DEFAULT_LOOK_BACK_DAYS}"
    )
    ...
    if (
        not api_url
        and not api_dev_url
    ) or not api_key or not openai_admin_key:

        logger.error(
            "Missing required environment variables. "
            "Required: "
            "API_URL or API_DEV_URL, "
            "API_KEY, "
            "OPENAI_ADMIN_KEY"
        )
    ...
    try:
        ...
        logger.info(
            "Step 1/4: Fetching OpenAI cost usage..."
        )

        cost_data = _fetch_openai_costs(
            openai_admin_key
        )

        logger.info(
            f"Fetched {len(cost_data)} cost buckets"
        )
        ...
        logger.info(
            "Step 2/4: Fetching OpenAI completions usage..."
        )

        completions_data = (
            _fetch_openai_completions(
                openai_admin_key
            )
        )
        ...
        logger.info(
            f"Fetched "
            f"{len(completions_data)} "
            f"completion buckets"
        )
        ...
        if api_dev_url:
            targets.append(api_dev_url)

        if not targets:
            raise Exception(
                "Neither API_URL nor API_DEV_URL "
                "is configured"
            )
            ...
            try:
                ...
                return {
                    ...
                    "body": json.dumps(
                        {
                            "message": (
                                f"Sync completed via {url}"
                            ),
                            "openai_requests": (
                                _openai_request_count
                            ),
                            "data": snapshot,
                        }
                    ),
        ...
        # All backend targets failed

        logger.error(
            f"All API endpoints failed. "
            f"OpenAI requests made: "
            f"{_openai_request_count}"
        )
